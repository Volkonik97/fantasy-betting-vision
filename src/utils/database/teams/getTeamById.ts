      ...
      try {
        ...
        if (playerError) {
          ...
          team.players = playerData.map(player => ({
            ...
            killParticipation: player.kill_participation_pct || 0, // Use kill_participation_pct instead of kill_participation
      ...
      try {
        ...
        if (playersError) {
          ...
          team.players = playersData.map(player => ({
            ...
            csPerMin: player.cspm || 0,
            damageShare: player.damage_share || 0,
            killParticipation: player.kill_participation_pct || 0, // Using kill_participation_pct instead of kill_participation
            championPool: player.champion_pool ? String(player.champion_pool) : ''
          }));
        } else {
          console.log(`No players found for team ${teamId}`);
