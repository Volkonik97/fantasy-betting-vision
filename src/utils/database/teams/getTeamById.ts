      ...
      try {
        ...
        if (playerError) {
          ...
          team.players = playerData.map(player => ({
            ...
            killParticipation: player.kill_participation || 0, // Use kill_participation instead of kill_participation_pct
      ...
      try {
        ...
        if (playersError) {
          ...
          team.players = playersData.map(player => ({
            ...
            csPerMin: player.cspm || 0,
            damageShare: player.damage_share || 0,
            killParticipation: player.kill_participation || 0, // Using kill_participation instead of kill_participation_pct
            championPool: player.champion_pool ? String(player.champion_pool) : ''
          }));
        } else {
          console.log(`No players found for team ${teamId}`);
